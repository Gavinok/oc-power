...
static const char HTML_PAGE[] =
    ...
    "<style>"
    "body{background:#111;color:#0f0;font:12px/1.4 monospace;margin:0;padding:44px 8px 8px}"
...
static int log_vprintf_hook(const char *fmt, va_list args)
{
    ...
    va_end(args2);
    if (entry.len < 0) entry.len = 0;
    if (entry.len >= (int)sizeof(entry.data)) entry.len = sizeof(entry.data) - 1;
...
void wifi_log_server_start(const char *ssid, const char *password)
{
    ...
    if (loop_err != ESP_ERR_INVALID_STATE) ESP_ERROR_CHECK(loop_err);
    ...
    wifi_config_t ap_cfg = {
        .ap = {
            ...
            .authmode = (password && strlen(password))
